...
def parse_money(text):
    ...
    if t == "":
        return np.nan
    # Estilo argentino: "." miles, "," decimales
    # quitamos espacios y signos extra
    t = t.replace(" ", "").replace("$", "")
    # Si contiene puntos como miles y coma decimal
    if re.search(r",\d{1,2}$", t):
        t = t.replace(".", "").replace(",", ".")
    # Si tiene más de un signo, conservamos el último
    try:
        return float(t)
    except Exception:
        # encontrar número dentro
...
def extract_tables_from_pdf(file_like) -> pd.DataFrame:
    """
    Devuelve un DataFrame con columnas: fecha, descripcion, debito, credito, saldo, pagina, fila
    No inventa registros: solo captura lo que encuentra en tablas.
    """
    records = []
    with pdfplumber.open(file_like) as pdf:
        for pageno, page in enumerate(pdf.pages, start=1):
            # dos estrategias: 1) tables (stream & lattice), 2) líneas por regex como fallback
            for table in page.extract_tables(
                ...
                for i, c in enumerate(cols):
                    if idx_fecha is None and re.search(r"fecha", c):
                        idx_fecha = i
                    ...
                    if idx_credito is None and re.search(r"cr[eé]dito|creditos?", c):
                        idx_credito = i
                    if idx_saldo is None and re.search(r"saldo", c):
                        idx_saldo = i
                # Si primera fila no parece encabezado, igual procesamos asumiendo posiciones comunes
                start_row = 1 if any([idx_fecha, idx_desc, idx_debito, idx_credito, idx_saldo]) else 0
                for ridx, row in enumerate(table[start_row:], start=start_row):
                    ...
                    # Si todo está vacío, salteamos
                    if not any([fecha, desc, deb, cred, sal]):
                        continue
                    records.append(
                        {
                            "fecha_raw": fecha,
    ...
    # quitar filas completamente NaN en importes
    if {"debito", "credito"}.issubset(df.columns):
...
def find_period_final_balance(pdf_file_like):
    """
    Busca expresiones "Saldo al dd/mm/yyyy" y su importe en la misma línea o siguiente.
    Devuelve dict con 'fecha' y 'monto' si lo encuentra; si no, None.
    """
    with pdfplumber.open(pdf_file_like) as pdf:
        ...
        if candidates:
            # nos quedamos con el último (fin de período)
            cand = candidates[-1]
            return cand
    return None
...
def clasificar(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Seguros u otros débitos automáticos
    ...
    # Aliquot detection for IVA lines
...
# -----------------------------
def construir_signo(df: pd.DataFrame, credito_resta=True):
    """
    Construye columna 'importe' con el signo correcto, según regla del usuario.
    - Si credito_resta=True -> 'credito' se toma como negativo y 'debito' como positivo.
    - Si credito_resta=False -> 'credito' suma y 'debito' resta.
    """
...
def calcular_saldos(df: pd.DataFrame, saldo_final_ref: float | None):
    """
    Si hay saldo_final_ref, reconstruye saldo inicial como: saldo_inicial = saldo_final_ref - sum(importe).
    """
...
def resumen_por_categorias(df: pd.DataFrame) -> dict:
    ...
    # Entre cuentas propias (pueden ser + o -)
    ...
    # SIRCREB y DyC (son retenciones/debitos)
    out["sircreb"] = -df.loc[df["tipo"].eq("SIRCREB") & (df["importe"]<0), "importe"].sum()
    out["dyc"]     = -df.loc[df["tipo"].eq("DyC") & (df["importe"]<0), "importe"].sum()
    # Comisiones + IVA por alicuota (sin inventar: suma lo que diga "COMISION" y lo que diga "IVA")
    ...
    out["iva_21"] = iva_21
    out["iva_105"] = iva_105
    # Percepciones IVA (suman saldo a favor del fisco normalmente como debito); respetamos el signo tal cual figura
...
def main():
    ...
    st.info("Regla aplicada por defecto: **Crédito resta, Débito suma.** Puedes cambiarla abajo si tu banco usa la convención opuesta.")
    credito_resta = st.toggle("Tomar **Crédito** como importe que **resta** y **Débito** como importe que **suma**.", value=True)

    uploaded = st.file_uploader("Subí un PDF del resumen bancario", type=["pdf"])

    default_path = "2508-NBSF-Creland.pdf"
    ...
    if uploaded is not None:
        ...
        file_like = open(default_path, "rb")
        file_source_name = default_path
        st.caption(f"Usando archivo por defecto: {default_path}")
    else:
        st.warning("Subí un PDF para continuar.")
        st.stop()

    # Extraer movimientos
    df = extract_tables_from_pdf(file_like)
    if len(df) == 0:
        st.error("No se detectaron tablas con movimientos. Probá con otro PDF o compárteme un ejemplo.")
        st.stop()

    # Clasificar y signo
    df = clasificar(df)
    df = construir_signo(df, credito_resta=credito_resta)

    # Buscar saldo final de período
    # Para esto necesitamos volver a abrir el fichero
    file_like.seek(0)
    ...
    # Orden lógico por fecha + pagina + fila
    ...
    st.divider()
    st.subheader("Gastos por comisiones e IVA (sin inventar)")
    ...
    # Detalle de transferencias con CUIT
    st.divider()
    ...
    st.caption("Nota: La app solo suma lo que el PDF declara. No infiere ni calcula netos a partir de brutos. Si el PDF no detalla el IVA, no se inventa.")
