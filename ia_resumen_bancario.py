...
# ---- Banco Galicia (regex & helpers) ----
GALICIA_HEADER_RE  = re.compile(r"\bFECHA\s+DESCRIPC(?:I[ÓO]N|ION)\s+ORIGEN\s+CR[ÉE]DITO\s+D[ÉE]BITO\s+SALDO\b", re.IGNORECASE)
GAL_SALDO_INICIAL_RE = re.compile(r"SALDO\s+INICIAL.*?((?:\d{1,3}(?:\.\d{3})*|\d+)\s?,\s?\d{2}-?)", re.IGNORECASE)
GAL_SALDO_FINAL_RE   = re.compile(r"SALDO\s+FINAL.*?((?:\d{1,3}(?:\.\d{3})*|\d+)\s?,\s?\d{2}-?)", re.IGNORECASE)
...
# ---------- Detección de banco (con prioridad Galicia explícita) ----------
...
def detect_bank_from_text(txt: str) -> str:
    ...
    # Prioridad dura si dice explícitamente BANCO GALICIA o RESUMEN DE CUENTA
    if "BANCO GALICIA" in U or "RESUMEN DE CUENTA" in U:
...
def parse_galicia_lines(lines: list[str]) -> pd.DataFrame:
    """
    Galicia: FECHA | DESCRIPCIÓN | ORIGEN | CRÉDITO | DÉBITO | SALDO
    Tomamos SIEMPRE penúltimo monto como movimiento con signo y último como saldo.
    """
    rows, seq = [], 0
    for ln in lines:
        s = ln.strip()
        ...
        saldo = normalize_money(am[-1].group(0))  # SALDO
        mov   = normalize_money(am[-2].group(0))  # MOVIMIENTO (con signo: + crédito, - débito)
...
def render_account_report_galicia(
    ...
    fecha_cierre, saldo_final_pdf = find_saldo_final_from_lines(lines)
    header_saldos = galicia_header_saldos_from_text(header_txt)

    # Sin movimientos
    if df.empty:
        saldo_inicial = float(header_saldos.get("saldo_inicial", np.nan)) if not np.isnan(header_saldos.get("saldo_inicial", np.nan)) else 0.0
        total_debitos = total_creditos = 0.0
        saldo_final_visto = float(header_saldos.get("saldo_final", np.nan)) if not np.isnan(header_saldos.get("saldo_final", np.nan)) else saldo_inicial
        saldo_final_calculado = saldo_inicial + total_creditos - total_debitos
        diferencia = saldo_final_calculado - saldo_final_visto
        cuadra = abs(diferencia) < 0.01
    ...
    # Débito/Crédito según signo del movimiento (tu regla)
    df = df.sort_values(["fecha", "orden"]).reset_index(drop=True)
    ...
    if not np.isnan(header_saldos.get("saldo_inicial", np.nan)):
        ...
        saldo_inicial = s0 - m0  # si m0>0 resta; si m0<0, resta el negativo (= suma)
    ...
    if not np.isnan(header_saldos.get("saldo_final", np.nan)):
        saldo_final_visto = float(header_saldos["saldo_final"])
    else:
    ...
    # ===== Resumen Operativo (25.413 neteado) =====
    iva21_mask  = df_sorted["Clasificación"].eq("IVA 21% (sobre comisiones)")
    iva105_mask = df_sorted["Clasificación"].eq("IVA 10,5% (sobre comisiones)")
    iva21  = float(df_sorted.loc[iva21_mask,  "debito"].sum())
    iva105 = float(df_sorted.loc[iva105_mask, "debito"].sum())
    ...
    percep_iva = float(df_sorted.loc[df_sorted["Clasificación"].eq("Percepciones de IVA") | mask_percep_iva, "debito"].sum())
    ...
    # Grilla más prolija (no a todo ancho, alto fijo)
    st.caption("Detalle de movimientos")
    show_cols = ["fecha","descripcion","debito","credito","saldo","Clasificación"]
    for c in show_cols:
        if c not in df_sorted.columns:
            df_sorted[c] = np.nan
    styled = df_sorted[show_cols].style.format({c: fmt_ar for c in ["debito","credito","saldo"]}, na_rep="—")
    st.dataframe(styled, use_container_width=False, height=560)
...
if _bank_name == "Banco Macro":
    ...
    # Galicia usa su parser y regla de saldo inicial (primer saldo ± primer movimiento)
    all_lines = [l for _, l in extract_all_lines(io.BytesIO(data))]
    txt_full  = _text_from_pdf(io.BytesIO(data))
    if not any(GALICIA_HEADER_RE.search(l) for l in all_lines):
        st.info("Galicia: no se encontró el encabezado de la tabla; se procesa por montos detectados.")
    render_account_report_galicia(_bank_slug, "Cuenta Corriente (Galicia)", "s/n", "galicia-unica", all_lines, txt_full)
