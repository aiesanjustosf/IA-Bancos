...
# Banco Macro (encabezados de cuentas)
...
def parse_lines(lines) -> pd.DataFrame:
    ...
    for ln in lines:
        ...
        monto   = normalize_money(am[-2].group(0))   # penúltima = monto (cuando existe; en Delta no se usa)
        ...
        rows.append({
            ...
            "debito": 0.0,
            "credito": 0.0,
            "importe": np.nan,     # se completa después
            "monto_pdf": monto,
            "saldo": saldo,
            "pagina": 0,
            "orden": seq
...
# ========== RENDER POR CUENTA (router Galicia vs Delta) ==========
def render_account_report(
    ...
    if banco_slug == "galicia":
        # débitos/créditos desde monto firmado
        ...
        # saldos: inicial del encabezado o reconstrucción con la primera fila
        ...
        # modo delta (Macro / Santa Fe / Nación / Santander)
    ...
    # Insertar SALDO ANTERIOR como primera fila informativa
    ...
    saldo_inicial_show = float(df_sorted.loc[0, "saldo"]) if "saldo" in df_sorted.columns else np.nan
    ...
    saldo_final_visto = float(df_sorted["saldo"].iloc[-1]) if (np.isnan(saldo_final_pdf) and "saldo" in df_sorted.columns) else float(saldo_final_pdf)
    ...
    iva21_mask  = df_sorted["Clasificación"].eq("IVA 21% (sobre comisiones)") if "Clasificación" in df_sorted.columns else pd.Series(False, index=df_sorted.index)
    iva105_mask = df_sorted["Clasificación"].eq("IVA 10,5% (sobre comisiones)") if "Clasificación" in df_sorted.columns else pd.Series(False, index=df_sorted.index)
    ...
    percep_iva = float(df_sorted.loc[df_sorted.get("Clasificación","").eq("Percepciones de IVA"), "debito"].sum()) if "debito" in df_sorted.columns else 0.0

    # Ley 25.413 con signo (créditos restan)
    ley_mask = df_sorted.get("Clasificación","").eq("LEY 25413")
    ...
    sircreb = float(df_sorted.loc[df_sorted.get("Clasificación","").eq("SIRCREB"), "debito"].sum()) if "debito" in df_sorted.columns else 0.0

    if banco_slug == "galicia":
        # fallback de IVA / percepciones por texto (cuando la clasificación no cubre todo)
        udesc = df_sorted["desc_norm"].fillna("").str.upper()
        mask_percep_iva = udesc.str.contains(r"PERCEP\.?\s*IVA")
        percep_iva = float(df_sorted.loc[mask_percep_iva, "debito"].sum()) if "debito" in df_sorted.columns else 0.0
        mask_iva_gal = udesc.str.contains(r"\bIVA\b") & (~mask_percep_iva)
        iva21_gal = float(df_sorted.loc[mask_iva_gal, "debito"].sum()) if "debito" in df_sorted.columns else 0.0
        if iva21_gal > 0:
            ...
            net21 = float(df_sorted.loc[mask_comisiones_fallback, "debito"].sum()) if "debito" in df_sorted.columns else 0.0
            iva21 = 0.0
    ...
    # Tabla (sin Styler para evitar errores en Streamlit Cloud)
    ...
    try:
        ...
        st.download_button(
            ...
            file_name=f"resumen_bancario_{banco_slug}_{re.sub(r'[^0-9A-Za-z]+', '_', account_number)}.xlsx",
        ...
        st.download_button(
            ...
            data=csv_bytes,
            file_name=f"resumen_bancario_{banco_slug}_{re.sub(r'[^0-9A-Za-z]+', '_', account_number)}.csv",
            mime="text/csv",
            use_container_width=True,
    ...
    if REPORTLAB_OK:
        try:
            ...
            st.download_button(
                ...
                file_name=f"Resumen_Operativo_IVA_{banco_slug}_{re.sub(r'[^0-9A-Za-z]+', '_', account_number)}.pdf",
...
_auto_bank = detect_bank_from_text(_bank_txt)
...
_bank_name = forced if forced != "Auto (detectar)" else _auto_bank
...
if _bank_name == "Banco Macro":
    ...
    if not any(GALICIA_HEADER_RE.search(l) for l in all_lines):
        st.info("No se encontró explícitamente el encabezado de la tabla de Galicia; se procesa igual por montos/saldos.")
