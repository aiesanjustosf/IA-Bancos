...
def parse_santander_lines(lines: list[str]) -> pd.DataFrame:
    ...
    for ln in lines:
        ...
        if len(am) < 2:
            # acá suelen estar líneas de "Saldo Inicial" (1 monto) -> las tratamos afuera
            continue
...
# Santander — comisiones e IVA (Ley 27743) – ampliado a "transfisc"
...
if _bank_name == "Banco Macro":
   ...
   # Meta visible
    ...
    # 1) Extraigo todas las líneas reales
    all_lines = [l for _, l in extract_all_lines(io.BytesIO(data))]

    # 2) Parser específico -> df_san
    df_san = parse_santander_lines(all_lines)

    # 3) Armo líneas sintéticas para reutilizar el render genérico
    synth_lines = []

    # 3.a Inyectar SALDO INICIAL (1 solo importe) desde el PDF real
    si_line = None
    for ln in all_lines:
        U = ln.upper()
        if "SALDO INICIAL" in U and _only_one_amount(ln) and DATE_RE.search(ln):
            # usamos la línea original pero simplificada: "<fecha> SALDO INICIAL <monto>"
    ...
    if df_san.empty:
        # Si falló el parser, caemos a genérico con líneas reales
        ...
        for _, r in df_san.iterrows():
            ...
            def mk(x):  # formato AR con signo al final para negativos
