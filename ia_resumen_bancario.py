...
BANK_GALICIA_HINTS   = ("BANCO GALICIA","RESUMEN DE CUENTA","DESCRIPCIÓN ORIGEN CRÉDITO DÉBITO SALDO","SIRCREB","IMP. DEB./CRE. LEY 25413","TRANSFERENCIA DE TERCEROS")
...
def parse_lines_generic(lines) -> pd.DataFrame:
    ...
    for ln in lines:
        ...
        rows.append({
            ...
            "importe": importe, "monto_pdf": importe,
...
def parse_santander_lines(lines: list[str]) -> pd.DataFrame:
    ...
    for ln in lines:
        ...
        rows.append({
            ...
            "importe": cre - deb, "monto_pdf": cre - deb,
            "saldo": saldo, "pagina": 0, "orden": seq
        })
        prev_saldo = saldo
...
def find_saldo_anterior_from_lines(lines):
    ...
    for i, ln in enumerate(lines):
        if re.search(r"SALDO\s+U?LTIMO\s+RESUMEN", ln, re.I):
...
def clasificar(desc: str, desc_norm: str, deb: float, cre: float) -> str:
    ...
    if RE_SANTANDER_COMISION_CUENTA.search(u) or RE_SANTANDER_COMISION_CUENTA.search(n): return "Gastos por comisiones"
    if RE_SANTANDER_IVA_TRANSFSC.search(u) or RE_SANTANDER_IVA_TRANSFSC.search(n):     return "IVA 21% (sobre comisiones)"
    if RE_SIRCREB.search(u) or RE_SIRCREB.search(n) or ("SIRCREB" in u) or ("SIRCREB" in n): return "SIRCREB"
    ...
    if ("COMIS.TRANSF" in u) or ("COMIS.TRANSF" in n) or ("COMIS TRANSF" in u) or ("COMIS TRANSF" in n) or \
       ("COMIS.COMPENSACION" in u) or ("COMIS.COMPENSACION" in n) or ("COMIS COMPENSACION" in u) or ("COMIS COMPENSACION" in n): return "Gastos por comisiones"
    if ("MANTENIMIENTO MENSUAL PAQUETE" in u) or ("MANTENIMIENTO MENSUAL PAQUETE" in n) or \
       ("COMOPREM" in n) or ("COMVCAUT" in n) or ("COMTRSIT" in n) or ("COM.NEGO" in n) or ("CO.EXCESO" in n) or ("COM." in n): return "Gastos por comisiones"
...
# ---------- Helper de UI (genérico y Galicia) ----------
...
def parse_lines_generic_for_galicia(lines) -> pd.DataFrame:
    # Igual a genérico, pero guardamos monto_pdf tal cual (+/-) para clasificar débito/crédito por signo
...
def render_account_report_generic(
    ...
    if not np.isnan(saldo_anterior):
        ...
        apertura = pd.DataFrame([{
            ...
            "saldo": float(saldo_anterior), "pagina": 0, "orden": 0
        }])
    ...
    fecha_cierre, saldo_final_pdf2 = fecha_cierre, saldo_final_pdf
    saldo_final_visto = float(df_sorted["saldo"].iloc[-1]) if np.isnan(saldo_final_pdf2) else float(saldo_final_pdf2)
    saldo_final_calculado = saldo_inicial + total_creditos - total_debitos
    diferencia = saldo_final_calculado - saldo_final_visto
    cuadra = abs(diferencia) < 0.01
...
def render_account_report_galicia(
    ...
    df = parse_lines_generic_for_galicia(lines)
    fecha_cierre, saldo_final_pdf = find_saldo_final_from_lines(lines)

    # Débito/Crédito por signo del monto_pdf
    df = df.sort_values(["fecha", "orden"]).reset_index(drop=True)
    df["debito"]  = np.where(df["monto_pdf"] < 0, -df["monto_pdf"], 0.0)
    df["credito"] = np.where(df["monto_pdf"] > 0,  df["monto_pdf"], 0.0)
    df["delta_saldo"] = df["saldo"].diff()

    # Saldos (encabezado preferente)
    ...
    # Resumen operativo (ajustes Galicia)
    st.caption("Resumen Operativo: Registración Módulo IVA")
    iva21_mask  = df_sorted["desc_norm"].fillna("").str.upper().str.contains(r"\bIVA\b")
    percep_mask = df_sorted["desc_norm"].fillna("").str.upper().str.contains(r"PERCEP\.?\s*IVA")
    iva21_mask  = iva21_mask & (~percep_mask)
    iva21 = float(df_sorted.loc[iva21_mask, "debito"].sum())
    net21 = round(iva21 / 0.21, 2) if iva21 else 0.0
...
# --- Flujo por banco (SEPARADO GALICIA vs RESTO) ---
if _bank_name == "Banco Macro":
    ...
    if df_san.empty:
        ...
        render_account_report_generic(_bank_slug, "Cuenta Corriente (Santander)", "s/n", "santander-unica", lines_for_render)
    else:
        ...
        render_account_report_generic(_bank_slug, "Cuenta Corriente (Santander)", "s/n", "santander-unica", synth_lines)
