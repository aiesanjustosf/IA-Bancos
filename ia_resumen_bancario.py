...
st.set_page_config(
    page_title="Analizador Universal de Resúmenes Bancarios",
    layout="wide",
...
# Busca el patrón de una línea que comienza con una fecha DD/MM/AA y sigue la estructura de tabla CSV/Texto.
# Grupos: [Fecha, Comprobante, Descripcion, Debito, Credito, Saldo]
PATRON_MOVIMIENTO = r'^\s*\"(\d{2}\/\d{2}\/\d{2})\"\s*,\s*\"?([^\"]*)\"?\s*,\s*\"?([^\"]*)\"?\s*,\s*\"?([\d\., ]*)\"?\s*,\s*\"?([\d\., ]*)\"?\s*,\s*\"?([\d\., ]*)\"?'

# Palabras clave para categorización (Genéricas. Se pueden editar y expandir)
CATEGORIAS = {
    "Ventas/Transferencias Recibidas": ["Transf", "Transfer", "Pago a Comercios", "Acreditac", "CABAL", "ECHO"],
    "Pagos a Proveedores/Servicios": ["C MS SA", "VAMAGRO SRL", "CH SISTEMAS SRL", "FILIPPA SILVIA GLADIS", "LOS NARANJOSSA", "PILAY SA", "TARJETA NARANJA"],
    "Gastos Financieros / Comisiones": ["Comision", "I.V.A. Debito Fiscal 21%", "Percepcion IVA RG 2408", "Servicio Modulo NyP"],
    "Impuestos y Percepciones": ["Impuesto Ley 25.413", "Percepcion IVA"],
    "Débitos y Cheques": ["Cheque de Camara", "Debito Inmediato (DEBIN)", "Debito/Credito Aut"],
    "Retiros/Extracciones": ["Contrasiento Liquidacion PROPINAS"],
}
...
def extract_movements_from_pdf_text(text):
    """
    Procesa el texto plano (extraído de un PDF) y extrae los movimientos financieros
    buscando los patrones de tabla y saldos.
    """
    ...
    # Se ajusta para capturar los movimientos que a veces están entre el encabezado y el final del resumen
    ...
    if not movements_block_match:
        st.error("❌ No se pudo identificar el bloque de movimientos (FECHA...SALDO) en el archivo. Verifica el formato.")
        return pd.DataFrame()
    ...
    for line in movements_text.split('\n'):
        ...
        if match:
            ...
            # Línea de continuación (para descripciones largas o sub-líneas de impuestos)
            if current_movement and len(current_movement) == 6:
                ...
                if continuation_match:
                    ...
                    # Verificamos si hay nuevos valores (para el formato de impuestos en varias líneas)
                    new_debito = continuation_match.group(2)
                    new_credito = continuation_match.group(3)
                    new_saldo = continuation_match.group(4)
                    
                    # Sobrescribir solo si el valor es positivo (>0) y el campo actual es cero (es la línea donde el valor aparece)
    ...
    # Filtrar movimientos con importe 0.0 que suelen ser líneas de texto sin valor (residuos de parseo).
    df = df[df['IMPORTE'] != 0.0].copy()

    # Añadir Saldo Anterior y Final
...
def load_and_process_file(uploaded_file):
    """Carga y procesa un archivo subido (con manejo del PDF en la demo)."""
    
    # --- SIMULACIÓN DE EXTRACCIÓN DE TEXTO DEL PDF (NECESARIO PARA ESTE ENTORNO) ---
    raw_text = None
    
    # Contenido del archivo subido inicialmente (CREDICOOP.pdf)
    CREDICOOP_CONTENT = st.session_state.get('credicoop_content')

    if uploaded_file.name == 'CREDICOOP.pdf' and CREDICOOP_CONTENT:
        # Usamos el contenido ya extraído para el archivo de la demo
        raw_text = CREDICOOP_CONTENT
    else:
        # Para un nuevo archivo, simulamos la lectura del texto que podría extraerse
        st.warning("⚠️ **Advertencia:** Para archivos PDF nuevos, este entorno no puede extraer el texto automáticamente (requiere librerías como pdfplumber). La aplicación asume que el archivo subido tiene un formato de tabla de texto similar al resumen inicial.")
        try:
             # Intentar leerlo como texto simple, si es TXT o la extracción falló
             raw_text = uploaded_file.getvalue().decode("utf-8")
        except Exception as e:
             st.error(f"No se pudo leer el archivo como texto: {e}")
             return pd.DataFrame()
...
def display_summary(df):
    """Muestra el resumen general de la cuenta y las métricas clave."""
    if df.empty:
    ...
    saldo_inicial = getattr(df, 'saldo_anterior', 0.0)
    saldo_final_calc = df['SALDO_CALCULADO'].iloc[-1] if not df.empty else saldo_inicial
    saldo_final_pdf = getattr(df, 'saldo_final', 0.0)

    total_ingresos = df[df['IMPORTE'] > 0]['IMPORTE'].sum()
    total_egresos = df[df['IMPORTE'] < 0]['IMPORTE'].sum() * -1 
...
def display_category_summary(df):
    ...
    st.subheader("Resumen de Flujos por Categoría 🏷️")
    ...
    egresos_summary['Total Egreso'] = egresos_summary['Total Egreso'] * -1 
...
def display_reconciliation(df):
    ...
    st.subheader("Control de Conciliación Bancaria ⚖️")
    st.info("La conciliación verifica que el **saldo final del resumen (SALDO_PDF)** coincida con el **saldo calculado (SALDO_CALCULADO)** sumando secuencialmente los movimientos al saldo inicial. Una 'Diferencia' distinta a cero indica un posible error de lectura o un saldo inicial incorrecto.")
    
    # Filtrar solo las líneas con diferencias significativas
    ...
    if reconciliation_df.empty:
        ...
        st.warning(f"⚠️ Se encontraron {len(reconciliation_df)} movimientos con diferencia de conciliación. Revise los detalles.")
...
# Inicializar el contenido del archivo inicial para la demo/pruebas
if 'credicoop_content' not in st.session_state:
    # Usamos el contenido extraído del archivo CREDICOOP.pdf como fallback
...
uploaded_files = st.sidebar.file_uploader(
    "Subir resúmenes bancarios (PDF o TXT)",
    type=["pdf", "txt"],
...
# Identificar el archivo a procesar (si subió múltiples, procesa el primero)
if uploaded_files:
    file_to_process = uploaded_files[0]
else:
    # Si no subió nada, usa el archivo de la demo como ejemplo
    if 'credicoop_content' in st.session_state:
        # Crea un objeto simulado para pasar a la función de procesamiento
...
if all_movements_df.empty:
    st.info("Por favor, sube uno o más archivos para comenzar el análisis. La herramienta funciona mejor con resúmenes donde la tabla de movimientos es fácilmente extraíble como texto plano.")
else:
    # 3. RESULTADOS
    ...
    # Filtros en la tabla de detalle
