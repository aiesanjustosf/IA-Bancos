...
BANK_SANTANDER_HINTS = ("BANCO SANTANDER","SANTANDER RIO","SALDO TOTAL","DETALLE IMPOSITIVO","SALDO INICIAL")
...
def detect_bank_from_text(txt: str) -> str:
    ...
    # Prioridad para Galicia para evitar falsos con Santander
...
# ---------- Clasificación ----------
RE_SIRCREB = re.compile(r"\bREGIMEN\s+DE\s+RECAUDACION\s+SIRCREB(?:\s+R)?\b|SIRCREB", re.IGNORECASE)
RE_PERCEP_RG2408 = re.compile(r"\bPERCEPCI[ÓO]N\s+IVA\s+RG\.?\s*2408\b", re.IGNORECASE)
RE_LEY25413 = re.compile(r"\b(?:IMPUESTO\s+)?LEY\s*25\.?413\b|IMPDBCR\s*25413|N/?D\s*DBCR\s*25413|IMP\.?\s*DEB\.?/?CRE\.?\s*LEY\s*25413", re.IGNORECASE)
...
def clasificar(desc: str, desc_norm: str, deb: float, cre: float) -> str:
    ...
    if RE_SIRCREB.search(u) or RE_SIRCREB.search(n) or ("ING. BRUTOS" in u and "CRED" in u) or ("ING. BRUTOS" in n and "CRED" in n):
        return "SIRCREB"
...
def render_account_report(
    ...
    if banco_slug == "galicia":
        # Galicia: usa signo del PDF (monto_pdf)
        ...
        # importe ya es monto_pdf
    else:
        # Resto: usa delta de saldo (independiente del signo del PDF)
        df["delta_saldo"] = df["saldo"].diff()
    ...
    if not np.isnan(saldo_inicial):
        ...
        fecha_apertura = (
            (first_date - pd.Timedelta(days=1)).normalize() + pd.Timedelta(hours=23, minutes=59, seconds=59)
    ...
    # Tabla (sin .style para evitar errores de Streamlit)
