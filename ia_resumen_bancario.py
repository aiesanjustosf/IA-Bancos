...
MONEY_RE_FUZZY  = re.compile(r"(?:\d+\s+)?\d{1,3}(?:\s?[.,]\s?\d{3})*(?:\s?[.,]\s?\d{2})(?:\s?-)?")
...
def clean_money_token(tok: str) -> str:
    ...
    neg = tok.endswith("-")
    tok = tok.rstrip("-")
    # elegir el ÚLTIMO separador como separador decimal
    last_sep = max(tok.rfind(","), tok.rfind("."))
    if last_sep != -1:
        main = tok[:last_sep].replace(".", "").replace(",", "").replace(" ", "")
        frac = tok[last_sep+1:].replace(" ", "")
        tok = main + "." + frac
    else:
    ...
    with pdfplumber.open(file_like) as pdf:
        for pageno, page in enumerate(pdf.pages, start=1):
            ...
            for raw in text.splitlines():
                ...
                desc = line[dmatch.end(): first_money.start()].strip() if first_money else line[dmatch.end():].strip()
                up = desc.upper()
...
def bucket_lines(words, tol=2.0):
    ...
    for w in sorted(words, key=lambda w: (w["top"], w["x0"])):
        if top is None or abs(w["top"] - top) <= tol:
            ...
            lines.append(cur); cur = [w]; top = w["top"]
...
deb = df[df["importe"]<0].copy(); deb["monto"] = -deb["importe"]
st.dataframe(deb[["fecha","descripcion","monto","tipo","cuit","pagina"]])

st.subheader("TODOS los Créditos")
cre = df[df["importe"]>0].copy(); cre["monto"] = cre["importe"]
