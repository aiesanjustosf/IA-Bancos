...
# ---- Banco Credicoop (hints + parser por XY) ----
CREDICOOP_HINTS = (
    "BANCO CREDICOOP",
...
def normalize_money(tok: str) -> float:
    """Normaliza $ con soporte de signo '−' (U+2212) y '-'."""
    if not tok:
        return np.nan
    tok = tok.strip()
...
def render_account_report_generic(
    ...
    try:
        ...
        st.download_button(
            ...
            file_name=f"resumen_bancario_{banco_slug}.xlsx",
        ...
        st.download_button(
            ...
            data=csv_bytes,
            file_name=f"resumen_bancario_{banco_slug}.csv",
            mime="text/csv",
            use_container_width=True,
...
def render_account_report_galicia(
    ...
    df = parse_lines(lines)  # trae monto_pdf y saldo
    fecha_cierre, saldo_final_pdf = find_saldo_final_from_lines(lines)

    # D/C por signo del PDF + fallback por delta de saldo
    df = df.sort_values(["fecha", "orden"]).reset_index(drop=True)
    ...
    # Saldo inicial: encabezado si está, o reconstrucción de la 1ª fila
    saldo_inicial = np.nan
    if header_saldos:
    ...
    # IVA (débitos que mencionan IVA y NO son percepciones)
