...
# ---- Santander helpers ----
def santander_cut_before_detalle(lines: list[str]) -> list[str]:
    cut = len(lines)
...
def santander_extract_saldos(lines: list[str]):
    saldo_inicial = np.nan
    saldo_final = np.nan
    for ln in lines:
        U = ln.upper()
        if ("SALDO INICIAL" in U) and len(list(MONEY_RE.finditer(ln))) == 1 and DATE_RE.search(ln):
            saldo_inicial = normalize_money(MONEY_RE.search(ln).group(0))
        if ("SALDO TOTAL" in U) and len(list(MONEY_RE.finditer(ln))) == 1:
            saldo_final = normalize_money(MONEY_RE.search(ln).group(0))
...
def normalize_money(tok: str) -> float:
    if not tok:
        return np.nan
    tok = tok.strip()
    neg = tok.endswith("-") or tok.startswith("-") or ("-" in tok and tok.replace(" ", "").startswith("-"))
...
def extract_all_lines(file_like):
    ...
    with pdfplumber.open(file_like) as pdf:
        for pi, p in enumerate(pdf.pages, start=1):
            ...
            out.extend([(pi, l) for l in combined if isinstance(l, str) and l.strip()])
    return out
...
def macro_split_account_blocks(file_like):
    ...
    for (pi, ln) in all_lines:
        ...
        if m_title:
            ...
            if m_same_line:
                nro = _normalize_account_token(m_same_line.group(1) if hasattr(m_same_line, "re") and m_same_line.re is RE_MACRO_ACC_NRO else m_same_line.group(0))
...
def render_account_report(
    ...
    # --- Débito / Crédito por banco ---
    if banco_slug == "galicia":
        # Galicia: el PDF trae el importe del movimiento (monto_pdf) con signo
        ...
        # Resto de bancos: el signo se infiere por el cambio de saldo
        df["delta_saldo"] = df["saldo"].diff()
        # 1ª fila: si no hay diff pero hay importe detectado, úsalo como delta
        if not df.empty and (pd.isna(df.loc[0, "delta_saldo"]) or df.loc[0, "delta_saldo"] == 0) and pd.notna(df.loc[0, "monto_pdf"]):
            df.loc[0, "delta_saldo"] = df.loc[0, "monto_pdf"]
        df["debito"]  = np.where(df["delta_saldo"] < 0, -df["delta_saldo"], 0.0)
        df["credito"] = np.where(df["delta_saldo"] > 0,  df["delta_saldo"], 0.0)
        df["importe"] = df["credito"] - df["debito"]  # signo humano (+ crédito / - débito)
    ...
    if banco_slug == "galicia":
        ...
        if np.isnan(saldo_inicial) and not df.empty:
            # reconstrucción con el primer movimiento con signo
    ...
    saldo_inicial_show = float(df_sorted.loc[0, "saldo"]) if not df_sorted.empty else 0.0
    total_debitos = float(df_sorted["debito"].sum())
    total_creditos = float(df_sorted["credito"].sum())
    saldo_final_visto = float(df_sorted["saldo"].iloc[-1]) if np.isnan(saldo_final_pdf) and not df_sorted.empty else (float(saldo_final_pdf) if not np.isnan(saldo_final_pdf) else 0.0)
    ...
    # Galicia: fallback de IVA/Percepciones
    if banco_slug == "galicia":
        udesc = df_sorted["desc_norm"].fillna("").str.upper()
        mask_percep_iva = udesc.str.contains(r"PERCEP\.?\s*IVA")
        percep_iva = max(percep_iva, float(df_sorted.loc[mask_percep_iva, "debito"].sum()))
...
def run_report_for_pdf(bank_name: str, data_bytes: bytes, bank_txt: str):
    slug = ("macro" if bank_name == "Banco Macro"
            else "santafe" if bank_name == "Banco de Santa Fe"
    ...
    if bank_name == "Banco Macro":
        blocks = macro_split_account_blocks(io.BytesIO(data_bytes))
        if not blocks:
            st.warning("No se detectaron encabezados de cuenta en Macro. Se intentará procesar todo el PDF (podría mezclar cuentas).")
            all_lines = [l for _, l in extract_all_lines(io.BytesIO(data_bytes))]
            render_account_report(slug, "CUENTA (PDF completo)", "s/n", "macro-pdf-completo", all_lines)
        else:
            st.caption(f"Información de su/s Cuenta/s: {len(blocks)} cuenta(s) detectada(s).")
            for b in blocks:
                render_account_report(slug, b["titulo"], b["nro"], b["acc_id"], b["lines"])
        return

    if bank_name == "Banco de Santa Fe":
        sf_accounts = santafe_extract_accounts(io.BytesIO(data_bytes))
        all_lines = [l for _, l in extract_all_lines(io.BytesIO(data_bytes))]
        if sf_accounts:
            ...
            for i, acc in enumerate(sf_accounts, start=1):
                ...
                render_account_report(slug, title, nro, acc_id, all_lines)
                if i < len(sf_accounts): st.markdown("")
        else:
            render_account_report(slug, "CUENTA", "s/n", "generica-unica", all_lines)
        return

    if bank_name == "Banco de la Nación Argentina":
        meta = bna_extract_meta(io.BytesIO(data_bytes))
        all_lines = [l for _, l in extract_all_lines(io.BytesIO(data_bytes))]
        titulo = "CUENTA (BNA)"
        ...
        txt_full = _text_from_pdf(io.BytesIO(data_bytes))
        _ = bna_extract_gastos_finales(txt_full)  # disponible si luego querés mostrarlo
        render_account_report(slug, titulo, nro, acc_id, all_lines)
        return

    if bank_name == "Banco Santander":
        all_lines = [l for _, l in extract_all_lines(io.BytesIO(data_bytes))]
        all_lines = santander_cut_before_detalle(all_lines)
        si, sf = santander_extract_saldos(all_lines)
        # si/sf se aplican automáticamente en render por detección de líneas (saldo anterior/final)
        render_account_report(slug, "Cuenta Corriente (Santander)", "s/n", "santander-unica", all_lines)
        return
    ...
    if bank_name == "Banco Galicia":
        ...
        header_saldos = galicia_header_saldos_from_text(bank_txt)
        render_account_report(slug, "Cuenta Corriente (Galicia)", "s/n", "galicia-unica", all_lines, header_saldos=header_saldos)
        return
