...
def lines_from_text(page) -> list[str]:
    txt = page.extract_text() or ""
    return [" ".join(l.split()) for l in txt.splitlines()]

def lines_from_words(page, ytol=2.0) -> list[str]:
...
# ====== helpers “a prueba de balas” ======
def safe_sort_df(df: pd.DataFrame) -> pd.DataFrame:
    for col, default in [("fecha", pd.NaT), ("orden", 0)]:
        if col not in df.columns:
            df[col] = default
    if df.empty:
        return df
    return df.sort_values(["fecha", "orden"], na_position="first").reset_index(drop=True)

def prefer_final_pdf_or_last(df: pd.DataFrame, saldo_final_pdf: float | None) -> float:
    if saldo_final_pdf is None or (isinstance(saldo_final_pdf, float) and np.isnan(saldo_final_pdf)):
        return float(df["saldo"].iloc[-1]) if ("saldo" in df.columns and not df.empty) else np.nan
    return float(saldo_final_pdf)

def compute_dc_galicia(df: pd.DataFrame) -> pd.DataFrame:
    df["debito"]  = np.where(df["monto_pdf"] < 0, -df["monto_pdf"], 0.0)
    df["credito"] = np.where(df["monto_pdf"] > 0,  df["monto_pdf"],  0.0)
    return df

def compute_dc_otros(df: pd.DataFrame) -> pd.DataFrame:
...
def derive_opening_from_first_row(df: pd.DataFrame) -> float:
    if df.empty or "saldo" not in df.columns:
        return np.nan
    s0 = float(df.iloc[0]["saldo"])
    if "monto_pdf" in df.columns and pd.notna(df.iloc[0]["monto_pdf"]):
        m0 = float(df.iloc[0]["monto_pdf"])
        return s0 - m0 if m0 > 0 else s0 + (-m0)
    if "delta_saldo" in df.columns and pd.notna(df.iloc[0]["delta_saldo"]):
        return float(s0 - df.iloc[0]["delta_saldo"])
    return np.nan
...
# ====== Clasificación (Resumen Operativo IVA) ======
...
# ====== Banco Santa Fe: extraer cuentas ======
...
def render_account_report(
    ...
    # 2) Saldos cabecera / cierre
    fecha_cierre, saldo_final_pdf = find_saldo_final_from_lines(lines)
    saldo_final_visto = prefer_final_pdf_or_last(df, saldo_final_pdf)

    # 3) Débitos / Créditos
    if banco_slug == "galicia":
        df = compute_dc_galicia(df)
    else:
        df = compute_dc_otros(df)

    # 4) Saldo inicial
    saldo_anterior = find_saldo_anterior_from_lines(lines)
    if pd.isna(saldo_anterior):
        saldo_inicial = derive_opening_from_first_row(df)
    else:
        saldo_inicial = float(saldo_anterior)

    if banco_slug == "galicia" and header_saldos:
        if not np.isnan(header_saldos.get("saldo_inicial", np.nan)):
            saldo_inicial = float(header_saldos["saldo_inicial"])
        # si header nos dio final válido, ya lo usamos arriba con prefer_final_pdf_or_last

    # 5) Insertar “Saldo anterior”
    if not np.isnan(saldo_inicial):
        ...
        apertura = pd.DataFrame([{
            ...
            "debito": 0.0, "credito": 0.0, "importe": 0.0, "monto_pdf": 0.0,
            "saldo": float(saldo_inicial), "pagina": 0, "orden": -1
        }])
    ...
    # 7) Totales y conciliación
    saldo_inicial_show   = float(df["saldo"].iloc[0]) if ("saldo" in df.columns and not df.empty) else np.nan
    ...
    cuadra = pd.notna(saldo_final_visto) and abs(diferencia) < 0.01
    ...
    ley_deb = float(df.loc[ley_mask, "debito"].sum())
    ley_cre = float(df.loc[ley_mask, "credito"].sum())
    ...
    if banco_slug == "galicia":
        ...
        iva21_gal = float(df.loc[mask_iva_gal, "debito"].sum())
        if iva21_gal > 0:
            ...
            net21 = float(df.loc[mask_comisiones_fallback, "debito"].sum())
            iva21 = 0.0
    ...
    df_sorted = df.drop(columns=["orden"], errors="ignore")
    df_sorted = safe_sort_df(df_sorted)
    styled = df_sorted.style.format({c: fmt_ar for c in ["debito","credito","importe","saldo"]}, na_rep="—")
    st.dataframe(styled, use_container_width=True)

    # 11) Descargas
    key_base = f"{banco_slug}_{re.sub(r'[^0-9A-Za-z]+','_',acc_id)}"
    try:
        import xlsxwriter
        output = io.BytesIO()
...
if _bank_name == "Banco Macro":
    ...
    if not blocks:
        st.warning("No se detectaron encabezados de cuenta en Macro. Se procesa todo el PDF (puede mezclar cuentas).")
        all_lines = [l for _, l in extract_all_lines(io.BytesIO(data))]
        render_account_report(_bank_slug, "CUENTA (PDF completo)", "s/n", "macro-pdf-completo", all_lines)
    else:
    ...
    bna_extras = bna_extract_gastos_finales(txt_full)  # disponible si querés mostrar
